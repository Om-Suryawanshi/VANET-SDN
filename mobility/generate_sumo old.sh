...
if [ "$SCENARIO" == "grid" ] || [ "$SCENARIO" == "all" ]; then
    ...
    netgenerate --grid --grid.x-number=5 --grid.y-number=5 --grid.length=150 --output-file grid.net.xml --no-turnarounds
...
EOF
    ...
    generate_traces highway.net.xml "--allow-fringe"
    cd ..
fi
...
if [ "$SCENARIO" == "city" ] || [ "$SCENARIO" == "all" ]; then
    ...
    wget -qO bhubaneswar.osm "https://overpass-api.de/api/map?bbox=85.8220,20.2680,85.8320,20.2780"
