...
void VehicleSdnApp::ReceiveBeacon(Ptr<Socket> socket)
{
    ...
    // The SendBeacon function put the IP string here
    std::string senderIpStr((char*)buffer.data()); 
    Ipv4Address senderCtrlIp(senderIpStr.c_str());

    Ipv4Address myCtrlIp = GetNodeIpv4Address(GetNode(), m_ctrlIfIndex);

    // Verify it is a valid IP and not ourselves
    if (senderCtrlIp != Ipv4Address::GetZero() && senderCtrlIp != myCtrlIp) {
        m_neighbors.insert(senderCtrlIp);
    }
}
...
void SdnControllerApp::ReceiveReport(Ptr<Socket> socket)
{
    ...
    // 1. Resolve Source Node (the car sending the report)
    Ipv4Address srcIp(srcStr.c_str());
    Ptr<Node> srcNode = FindNodeByCtrlIp(srcIp);

    if (!srcNode) {
        NS_LOG_WARN("ReceiveReport: Source IP " << srcIp << " unknown.");
        return;
    }

    uint32_t srcId = srcNode->GetId();
    m_topology[srcId].clear(); // Reset neighbors for this node

    // Get Source Position for distance calculation
    Ptr<MobilityModel> srcMobility = srcNode->GetObject<MobilityModel>();
    if (!srcMobility) return; 

    // 2. Process Reported Neighbors
    std::string nbrStr;
    while (ss >> nbrStr) {
        if (!nbrStr.empty() && nbrStr.find('.') != std::string::npos) {
            ...
            if (nbrNode) {
                ...
                Ptr<MobilityModel> nbrMobility = nbrNode->GetObject<MobilityModel>();
                if (!nbrMobility) continue;

                // --- CRITICAL FIX: DISTANCE CHECK ---
                double dist = srcMobility->GetDistanceFrom(nbrMobility);

                // Only allow link if distance is within Data Radio range (approx 250m)
                // If we don't check this, the Controller creates "impossible" long links.
                if (dist <= 180.0) {
                    uint32_t nbrId = nbrNode->GetId();
                    m_topology[srcId].insert(nbrId);
                    m_topology[nbrId].insert(srcId);
                } else {
                    // Optional: Log that we rejected a link to verify logic works
                    // NS_LOG_LOGIC("Link rejected: " << srcIp << "<->" << nbrIp << " Dist: " << dist);
                }
            } else {
...
{
    std::map<uint32_t, uint32_t> nextHop; // Stores NodeID -> NodeID
...
void SdnControllerApp::RecomputeRoutes()
{
    ...
    // Iterate over EVERY node in the simulation to calculate its specific routing table
    for (uint32_t i = 0; i < g_allNodes->GetN(); ++i) 
    {
        ...
        // This returns map<Destination_NodeId, NextHop_NodeId>
        std::map<uint32_t, uint32_t> nh = ComputeNextHop(m_topology, nodeId);

        std::ostringstream msg;
        for (auto &p : nh) {
            uint32_t dstId = p.first;   // Destination ID
            uint32_t nhId  = p.second;  // Next Hop ID
        ...
        // 4. Send this routing table to the specific Node's Control IP
...
int main(int argc, char *argv[])
{
    ...
    std::string xmlOutput = "sdn-results.xml";
    uint32_t seed = 1;
    uint32_t run = 1;
    double speedMs = 10.0;
    double simTime = 60.0;
    ...
    cmd.AddValue("protocol", "Routing Protocol: SDN, AODV, OLSR, DSDV", protocol);
    cmd.AddValue("output", "XML output filename", xmlOutput);
    ...
    */
    ...
    // --- INSERT THIS NEW MOBILITY BLOCK ---
    MobilityHelper mobility;

    // 1. Place nodes in a Grid initially (so they aren't all at 0,0)
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
        "MinX", DoubleValue(0.0),
        "MinY", DoubleValue(0.0),
        "DeltaX", DoubleValue(50.0), // 50m apart
        "DeltaY", DoubleValue(50.0),
        "GridWidth", UintegerValue(10), // 10x5 grid
        "LayoutType", StringValue("RowFirst"));

    // 2. Make them move using Random Waypoint
    // std::stringstream ssSpeed;
    // ssSpeed << "ns3::UniformRandomVariable[Min=" << (speedMs/2) << "|Max=" << speedMs << "]";

    std::stringstream ssSpeed;
    ssSpeed << "ns3::ConstantRandomVariable[Constant=" << speedMs << "]";
    ...
    // Define the simulation area (1000m x 1000m as per paper)
    ...
    // Fix Controller at center (500, 500)
    Ptr<MobilityModel> mm0 = nodes.Get(0)->GetObject<MobilityModel>();
    mm0->SetPosition(Vector(500.0, 500.0, 0.0));
    ...
    dataChannel.AddPropagationLoss("ns3::FriisPropagationLossModel"); 
    dataChannel.AddPropagationLoss("ns3::RangePropagationLossModel", 
                                   "MaxRange", DoubleValue(180.0));
    ...
    if (protocol == "SDN") 
    {
        NS_LOG_INFO("Installing SDN Controller and Apps...");
    ...
    {
        NS_LOG_INFO("Running traditional protocol: " << protocol);
    }
    ...
    */
    ...
    monitor->SerializeToXmlFile(xmlOutput, true, true);
